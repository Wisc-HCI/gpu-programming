...
export default function BlocklyInterface(props) {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
          ...
          try {
            ...
            //workspaceRef.render()
            //console.log("Blocks loaded into workspace:", workspaceRef.current.getAllBlocks());
          } catch (err) {
            console.error('Failed to parse XML Blocks:', err);
            alert('Error loading blocks: ' + err.message);
          }
      };
  ...
  useEffect(() => {
    ...
    const existingStartBlocks = workspaceRef.current.getAllBlocks().filter(block => block.type === 'Start');
